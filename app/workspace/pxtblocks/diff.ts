...
export function diffXml(oldXml: string, newXml: string, options?: DiffOptions): DiffResult {
    const oldWs = loadWorkspaceXml(oldXml, true);
...
function diffWorkspaceNoEvents(oldWs: Blockly.Workspace, newWs: Blockly.Workspace, options?: DiffOptions): DiffResult {
    ...
    const ws = initRenderingWorkspace();
    const newXml = saveWorkspaceXml(newWs, true);
    domToWorkspaceNoEvents(Blockly.utils.xml.textToDom(newXml), ws);
...
function findBlockIdByPosition(sourceMap: pxt.blocks.BlockSourceInterval[], loc: { start: number; length: number; }): string {
